...
import { ArrowLeft, Play, Camera, Image as ImageIcon, Maximize2 } from 'lucide-react';
import Footer from './Footer';

gsap.registerPlugin(ScrollTrigger);

export default function GalleryPage({ onBackHome }) {
    ...
    <div className="min-h-screen bg-black text-white font-outfit overflow-x-hidden" ref={containerRef}>
      ...
      <section ref={horizontalRef} className="h-screen flex items-center bg-[#050505] overflow-hidden">
        <div className="flex gap-4 sm:gap-8 px-6 sm:px-20 horizontal-track will-change-transform">
          {horizontalItems.map((item, index) => {
            ...
            return (
              <div key={item} className="w-[85vw] sm:w-[80vw] md:w-[60vw] h-[50vh] sm:h-[60vh] shrink-0 glass rounded-[30px] sm:rounded-[40px] overflow-hidden relative group border border-white/10">
      ...
      <section ref={gridRef} className="py-32 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {gridItems.map((item) => (
              <div key={item} className="gallery-grid-item aspect-square glass rounded-3xl overflow-hidden relative group border border-white/5">
                <img src={`/gallery-${item}.png`} alt={`Gallery ${item}`} className="w-full h-full object-cover opacity-50 group-hover:opacity-100 transition-all duration-500" />
      ...
      <section className="py-20 sm:py-32 bg-[#050505] text-center px-6">
        <div className="max-w-3xl mx-auto space-y-6 sm:space-y-10">
          ...
          <button className="bg-gradient-to-r from-neon-blue via-neon-purple to-neon-pink px-8 py-4 sm:px-12 sm:py-5 rounded-full text-white font-syncopate font-black text-[10px] sm:text-xs tracking-widest hover:shadow-[0_0_50px_rgba(112,0,255,0.4)] transition-all hover:scale-105">
            Book DJ Ali Max
          </button>
        </div>
      </section>
